@extends('layouts.frontend')
{{-- SEO --}}
{{-- SEO --}}
@php
    $page = (int) request('page', 1);

    $title = $page > 1
        ? "Startup Clinic Blog — Page {$page}"
        : "Startup Clinic Blog — Insights for Founders & SMEs";

    $desc  = "Expert articles on entrepreneurship, branding, digital marketing, websites, apps, and business growth from Startup Clinic Zambia.";

    $canonical = $page > 1
        ? request()->url().'?page='.$page
        : request()->url();
@endphp

@section('meta_title', $title)
@section('meta_description', $desc)
@section('canonical', $canonical)
@section('og_type', 'website')
@section('og_image', asset('assets/img/og/startup-clinic.jpg'))
@section('meta_keywords', 'startup zambia blog, business consulting, branding, digital marketing, SMEs, founders, entrepreneurship zambia')

{{-- JSON-LD for Blog Listing --}}
@push('structured_data')
@php
    $blogList = [
        '@context' => 'https://schema.org',
        '@type' => 'ItemList',
        'name' => 'Startup Clinic Blog',
        'numberOfItems' => $blogs->count(),
        'itemListElement' => [],
    ];

    foreach ($blogs as $i => $b) {
        $img = $b->featured_image
            ? asset('storage/'.$b->featured_image)
            : asset('assets/img/og/startup-clinic.jpg');

        $blogList['itemListElement'][] = [
            '@type' => 'ListItem',
            'position' => ($blogs->firstItem() ?? 1) + $i,
            'item' => [
                '@type' => 'BlogPosting',
                'headline' => $b->title,
                'url' => route('blog.show', $b->slug),
                'image' => [$img],
                'datePublished' => optional($b->published_at)->toIso8601String(),
                'author' => [
                    '@type' => 'Organization',
                    'name' => 'Startup Clinic Zambia'
                ],
            ],
        ];
    }

    $breadcrumbs = [
        '@context' => 'https://schema.org',
        '@type' => 'BreadcrumbList',
        'itemListElement' => [
            [
                '@type' => 'ListItem',
                'position' => 1,
                'name' => 'Home',
                'item' => url('/')
            ],
            [
                '@type' => 'ListItem',
                'position' => 2,
                'name' => 'Blog',
                'item' => route('blog.index')
            ],
            $blogs->currentPage() > 1 ? [
                '@type' => 'ListItem',
                'position' => 3,
                'name' => 'Page '.$blogs->currentPage(),
                'item' => request()->fullUrl()
            ] : null,
        ],
    ];

    // remove null breadcrumb entries
    $breadcrumbs['itemListElement'] = array_values(array_filter($breadcrumbs['itemListElement']));
@endphp

<script type="application/ld+json">
{!! json_encode($blogList, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) !!}
</script>

<script type="application/ld+json">
{!! json_encode($breadcrumbs, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) !!}
</script>
@endpush


@section('content')

<!-- banner -->
<div class="mil-inner-banner mil-p-0-120">
    <div class="mil-banner-content mil-up">
        <div class="mil-animation-frame">
            <div class="mil-animation mil-position-4 mil-dark mil-scale"
                 data-value-1="6" data-value-2="1.4"></div>
        </div>
        <div class="container">

            <ul class="mil-breadcrumbs mil-mb-60">
                <li><a href="{{ route('home') }}">Homepage</a></li>
                <li><a href="{{ route('blog.index') }}">Blog</a></li>
            </ul>

            <h1 class="mil-mb-60">
                Exploring <span class="mil-thin">the World</span><br>
                Through Our <span class="mil-thin">Blog</span>
            </h1>

            <a href="#blog" class="mil-link mil-dark mil-arrow-place mil-down-arrow">
                <span>Publications</span>
            </a>
        </div>
    </div>
</div>
<!-- banner end -->


<!-- popular -->
<section class="mil-soft-bg" id="blog">
    <div class="container mil-p-120-60">

        <div class="row align-items-center mil-mb-30">
            <div class="col-lg-6 mil-mb-30">
                <h3 class="mil-up">Popular Publications:</h3>
            </div>

            <div class="col-lg-6 mil-mb-30">
                <div class="mil-adaptive-right mil-up">
                    <a href="{{ route('blog.index') }}" class="mil-link mil-dark mil-arrow-place">
                        <span>View all</span>
                    </a>
                </div>
            </div>
        </div>

        <div class="row">

            @foreach($blogs->take(2) as $blog)
                <div class="col-lg-6">

                    <a href="{{ route('blog.show', $blog->slug) }}" 
                       class="mil-blog-card mil-mb-60">

                        <div class="mil-cover-frame mil-up">
                            <img src="{{ asset('storage/' . $blog->featured_image) }}" 
                                 alt="{{ $blog->title }}">
                        </div>

                        <div class="mil-post-descr">
                            <div class="mil-labels mil-up mil-mb-30">
                                <div class="mil-label mil-upper mil-accent">
                                    {{ $blog->category->name ?? 'General' }}
                                </div>
                                <div class="mil-label mil-upper">
                                    {{ $blog->published_at->format('M d Y') }}
                                </div>
                            </div>

                            <h4 class="mil-up mil-mb-30">{{ $blog->title }}</h4>

                            <p class="mil-post-text mil-up mil-mb-30">
                                {{ Str::words($blog->excerpt, 40, '...') }}
                            </p>

                            <div class="mil-link mil-dark mil-arrow-place mil-up">
                                <span>Read more</span>
                            </div>
                        </div>

                    </a>

                </div>
            @endforeach

        </div>

    </div>
</section>
<!-- popular end -->


<!-- categories + blog list -->
<section>
    <div class="container mil-p-120-120">

        <div class="row align-items-center mil-mb-30">

            <div class="col-lg-4 mil-mb-30">
                <h3 class="mil-up">///</h3>
            </div>

            <div class="col-lg-8 mil-mb-30">
                <div class="mil-adaptive-right mil-up">

                    <ul class="mil-category-list">

                        @foreach(\App\Models\BlogCategory::all() as $cat)
                            <li>
                                <a href="{{ route('blog.index', ['category' => $cat->slug]) }}"
                                   class="{{ request('category') == $cat->slug ? 'mil-active' : '' }}">
                                    {{ Str::of($cat->name)->explode(' ')->first() }}
                                </a>
                            </li>
                        @endforeach

                        <li>
                            <a href="{{ route('blog.index') }}"
                               class="{{ request('category') ? '' : 'mil-active' }}">
                                All categories
                            </a>
                        </li>

                    </ul>

                </div>
            </div>

        </div>

        <div class="row">

            @foreach($blogs as $blog)
                <div class="col-lg-12">

                    <a href="{{ route('blog.show', $blog->slug) }}"
                       class="mil-blog-card mil-blog-card-hori mil-more mil-mb-60">

                        <div class="mil-cover-frame mil-up">
                            <img src="{{ asset('storage/' . $blog->featured_image) }}"
                                 alt="{{ $blog->title }}">
                        </div>

                        <div class="mil-post-descr">

                            <div class="mil-labels mil-up mil-mb-30">
                                <div class="mil-label mil-upper mil-accent">
                                    {{ $blog->category->name ?? 'General' }}
                                </div>
                                <div class="mil-label mil-upper">
                                    {{ $blog->published_at->format('M d Y') }}
                                </div>
                            </div>

                            <h4 class="mil-up mil-mb-30">{{ $blog->title }}</h4>

                            <p class="mil-post-text mil-up mil-mb-30">
                                {{ Str::words($blog->excerpt, 60, '...') }}
                            </p>

                            <div class="mil-link mil-dark mil-arrow-place mil-up">
                                <span>Read more</span>
                            </div>

                        </div>

                    </a>

                </div>
            @endforeach

            <!-- pagination -->
            <div class="col-lg-12">
                <div class="mil-pagination">
                    {!! $blogs->links('pagination::bootstrap-4') !!}
                </div>
            </div>

        </div>

    </div>
</section>
<!-- blog end -->


<!-- call to action -->
<section class="mil-soft-bg">
    <div class="container mil-p-120-120">

        <div class="row">
            <div class="col-lg-10">
                <span class="mil-suptitle mil-suptitle-right mil-suptitle-dark mil-up">
                    Looking to make your mark? We'll help you turn 
                    your project into a success story.
                </span>
            </div>
        </div>

        <div class="mil-center">
            <h2 class="mil-up mil-mb-60">
                Stay up-to-date <span class="mil-thin">with the</span><br>
                latest news by <span class="mil-thin">subscribing</span><br>
                to our <span class="mil-thin">newsletter!</span>
            </h2>

            <div class="row justify-content-center mil-up">
                <div class="col-lg-4">
                    <form 
    method="POST" 
    action="{{ route('newsletter.subscribe') }}" 
    class="mil-subscribe-form mil-subscribe-form-2 mil-up"
>
    @csrf

    <!-- Honeypot field -->
    <input type="text" name="bot_field" style="display:none">

    <input 
        type="email" 
        name="email" 
        placeholder="Enter your email"
        required
    >

    <button 
        type="submit" 
        class="mil-button mil-icon-button-sm mil-arrow-place">
    </button>
</form>

@if(session('success'))
    <p class="text-success mt-2">{{ session('success') }}</p>
@endif

@error('email')
    <p class="text-danger mt-2">{{ $message }}</p>
@enderror

                </div>
            </div>
        </div>

    </div>
</section>
<!-- call to action end -->

@endsection
